...
impl AimEvaluator {
    const WIDE_ANGLE_MULTIPLIER: f64 = 1.45; 
    ...
    fn evaluate_diff_of<'a>(
            ...
            < 1.25 * osu_curr_obj.strain_time.min(osu_last_obj.strain_time)
        {
                ...
                .zip(osu_last_last_obj.angle)
            {
                ...
                // Holding repetitive aim patterns at these speeds is genuinely hard.
                ...
                // Scale penalty down as BPM rises; at 500+ BPM the penalty is gone
                let wide_penalty = wide_rep_raw * (1.0 - high_bpm_t);
                // Add repetition buff that ramps in — at 500 BPM, +15% bonus for repeated wide patterns
                let wide_rep_buff = high_bpm_t * 0.15;
                wide_angle_bonus *= angle_bonus
                    * ((1.0 - wide_penalty + wide_rep_buff).max(0.0));

                // * Acute angles: same treatment — penalty fades, buff ramps at high BPM.
